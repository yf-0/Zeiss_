\documentclass[journal,twoside,web]{ieeecolor_yf}
\usepackage{generic}
...
\begin{document}
\title{A Robotic- and AR-Assisted Human Machine Interface in Context of Neurosurgery}
\author{Fang You, Second B. Author, and Third C. Author, Jr., \IEEEmembership{Member, IEEE}
\thanks{This work is supported by EASY COHMO, a joint research project funded by the Federal Ministry of Education and Research of Germany.}
\thanks{F. You is a PhD candidate at the Department of Corporate Research \& Technology at Carl Zeiss AG (e-mail: Fang.You@Zeiss.com).}}
\maketitle
...
\begin{IEEEkeywords}
Human computer interaction, Robot, Robot motion, Surgery, Neurosurgery, Gesture recognition, Augmented reality, Three-dimensional displays, Microscopy.
\end{IEEEkeywords}

\section{Introduction}
\label{sec:introduction}
\IEEEPARstart{I}{n} neurological surgeries, it is critical to ensure reliable and high-quality vision of the surgical field. The optical microscope is the key instrument for amplified visualization of the surgical field to the surgeon.

which are hierarchical integrated into a hierarchical

xxxx xxxx xxxxx xxxxx xxxxxx xxxxxxxxxx xxxxxxxxxx xxxxxxxxx xxxx xxxxxxx xxxxxx xx xxxxxx xxxxx xxxxxx xxxxxxxxx
\subsection{Motivation}
reliable and intuitive touchless control - avoid contamination and 

free surgeon from microscope

improve perception experience 

\subsection{Problem Statement}
In an operation room (OR).......

surgeon handle microscope manually to position
...
\label{sec:sota}
The integrated human machine interface involves various functional components. There exist several studies and products regarding those functionalities and their respective hardware or algorithmic implementations.

\subsection{Surgical Microscope}
Carl Zeiss Kinevo 900

what it does

stereo vision / epipolar vision 

joints, robot, motions, Point in interest (PoI)
\begin{equation}
T_{pose}=T_{PoI}\cdot T_{orientation}\cdot T_{focus}^{-1}\label{eq}
\end{equation}
...
\subsubsection{dfawfwae sdagfwfa fwaefa fewa}

subsub, subsub, subsub, subsub, subsub, subsub, subsub, subsub, .

\subsection{Augmented Reality}
The implementation of augmented reality in surgical application can be traced back to the 1990s. 


Environment modeling / augmentation

localization

tracking with marker\cite{4649902}

Markerless tracking of the environment \cite{1634325}

image projection in real world

Head mounted desplay, latency

Hologram

AR in surgical application

\subsection{Touchless Interaction}
Traditional human-computer interaction (HCI) mostly involves mechanical interaction with the hardware. Touchless interaction refers to visual- or acoustic-based interactions, where mechanical interaction is excluded.
\subsubsection{Gestures}
Gesture recognition is an essential part of motion capture technology. 
A gesture is understood as an apparent body posture or motion \cite{4154947}. It can refer to motion of the whole body as well as particular components such as the head \cite{BetterThanYouThink}\cite{7533744}, the arms or the hands. 

As stated in the survey of Cheng et al. \cite{7208833}, using static or dynamic hand gestures is among the most capable and intuitive mediums of touchless human-computer interaction. Interaction with Hand gesture also gained its popularity in robotic application, such as gesture-based robot control \cite{7528373}. 
Both  hand gestures are defined in the corresponding datasets \cite{Ruffieux2014}. 

Hand gestures in an RGB- or/and depth-image can be acquired form a conventional RGB camera or more specialized devices such as the Microsoft Kinect sensor \cite{6333871} or Leap Motion controller (LMC) \cite{Marin2016}\cite{weichert2013analysis}\cite{7931886}. The recognition can be carried out regarding direct image features such as SURF \cite{6460664}, or interpreted features such as the estimated joint states with over 20 DoF \cite{6126483}. The joint-state-based Handicraft-Gesture Dataset \cite{7509645} illustrates numerous dynamic gestures, which are distinct and intuitive hand actions in morden life.  

Gesture control in HCI is implemented with various methods, which range from binary input of a predefined vocabulary \cite{5546423}\cite{7892569} to concurrent control with position and motion tracking. 
Human motion tracking may correspond to objects in numerous levels:  
\begin{itemize}
  \item position of items (e.g. hands) regarding the sensor \cite{7125339}
  \item relative position of items regarding their superior component (e.g. hands regarding torso) \cite{6209239}
  \item specific joint states of connected components (e.g. arm pose regarding torso, with joints of wrist, elbow and shoulder) \cite{7900051}
  \item dynamic joint states gesture with location information \cite{7559167}
\end{itemize}

To be noticed, Graßhoff et al. \cite{7559167} achieved a comprehensive control based on real-time 3D joint-based gesture tracking. The arm and hand gestures are geometrically projected to the robotic counterpart.   

\subsubsection{Speech}
The topic of speech-based hardware control in surgical context is gaining its popularity in the past few years \cite{7737032}\cite{7554323}\cite{5202719}.
One common application of speech control is the input of predefined commands, followed by the execution of a predefined action. 

Additionally, speech recognition can be extended to functions such as speaker identification \cite{365379} and emotion detection \cite{7226047}.  
...
intension prediction

\subsection{Surgical Workflow}
read the paper of Luzie 
...
\bibliography{this_lib}
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{_MG_5008-.jpg}}]{Fang You} received the B.Sc. degree in Automation \& Control Engineering from Universität Duisburg-Essen, Duisburg, Germany in 2014 
and the M.Ss. degree in Computer Science with the specializations of \textit{Robotic \& Automation} and \textit{Anthropomatik \& Cognitive Systems} from Karlsruher Institut für Technologie, Karlsruhe, Germany in 2017. He is currently a PhD candidate at the Corporate Research \& Technology Department at Carl Zeiss AG, Oberkochen, Germany.
\end{IEEEbiography}
